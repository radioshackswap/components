...
import { CETH, Token, Trade, TradeType } from '@radioshackswap/sdk2';
...
const ConfirmLimitOrderDrawer: React.FC<Props> = (props) => {
  ...
  const getInputCurrency = () => {
    if (inputCurrency1 && inputCurrency1?.symbol === CETH[chainId].symbol) {
      return CETH[chainId];
  ...
  const getOutputCurrency = () => {
    if (outputCurrency1 && outputCurrency1?.symbol === CETH[chainId].symbol) {
      return CETH[chainId];
