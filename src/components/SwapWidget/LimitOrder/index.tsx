...
import { CETH, JSBI, Token, TokenAmount, Trade } from '@radioshackswap/sdk2';
...
import { useChainId, useRadioShackWeb3 } from 'src/hooks';
...
const LimitOrder: React.FC<Props> = ({
  ...
  const getInputCurrency = () => {
    if (gelatoInputCurrency && gelatoInputCurrency?.symbol === CETH[chainId].symbol) {
      return CETH[chainId];
    } else if (inputTokenInfo && inputTokenInfo?.symbol === CETH[chainId].symbol) {
      return CETH[chainId];
  ...
  const getOutputCurrency = () => {
    if (gelatoOutputCurrency && gelatoOutputCurrency?.symbol === CETH[chainId].symbol) {
      return CETH[chainId];
    } else if (outputTokenInfo && outputTokenInfo?.symbol === CETH[chainId].symbol) {
      return CETH[chainId];
  ...
  const onCurrencySelect = useCallback(
    (currency: any, tokenDrawerTypeArg?: LimitNewField) => {
      ...
      if (currency?.symbol === CETH[chainId].symbol) {
