import { CETH, ChainId, Currency, Token, currencyEquals } from '@radioshackswap/sdk2';
...
const currencyKey = (columnIndex: number, rowIndex: number, data: Currency[], chainId: ChainId): string => {
  ...
  return currency instanceof Token
    ? currency.address
    : currency === CETH[chainId] && CETH[chainId]?.symbol
    ? (CETH[chainId]?.symbol as string)
    : '';
};
...
const SelectTokenDrawer: React.FC<Props> = (props) => {
  ...
  const filteredTokens: Token[] = useMemo(() => {
    ...
    tokens.unshift(CETH[chainId] as Token);
  ...
  const currencies = useMemo(() => {
    if (searchQuery === '') {
      ...
      const _tokens = filteredSortedTokens.filter((token) => token !== CETH[chainId]);
      // Need to check when implement near
      // return CHAINS[chainId]?.evm ? [CETH[chainId], ..._tokens] : [..._tokens];
      return [CETH[chainId], ..._tokens];
    }
    return filteredSortedTokens;
  }, [filteredSortedTokens, chainId]);
  //const currencies = useMemo(() => [Currency.CETH, ...filteredSortedTokens], [filteredSortedTokens]);
