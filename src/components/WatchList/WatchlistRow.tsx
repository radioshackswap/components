...
const WatchlistRow: React.FC<Props> = ({ coin, onClick, onRemove, isSelected }) => {
  ...
  const radioshackData = useTokenWeeklyChartData(coin?.address?.toLowerCase());

  const chartData = coinGekoData.length > 0 ? coinGekoData : radioshackData;
  ...
  return (
    ...
    >
      ...
      <Box textAlign="right" minWidth={30} height={'100%'}>
        {showDeleteButton && coin.address !== RADIO[chainId].address && (
