import { ALL_CHAINS, AVALANCHE_MAINNET, CETH, ChainId, Currency, Pair, Token, TokenAmount } from '@radioshackswap/sdk2';
...
export function useGetWalletChainTokens(chainId: number) {
  ...
  const getRadioShackPairs = async () => {
    const query = qs.stringify(
      {
        id: account,
        protocol_id: 'avax_radioshack',
      },
      {
        encodeValuesOnly: true,
      },
  ...
  const getTokens = async () => {
    if (account) {
      ...
      let requestTokens: (TokenDataUser | PairDataUser)[] = data
        ...
        .map((token: any) => {
          if (token?.id?.toLowerCase() === (CETH[chainId]?.symbol).toLowerCase()) {
            return new TokenDataUser(CETH[chainId], token?.price, token?.amount);
          }
      ...
      if (chainId === ChainId.AVALANCHE) {
        const pairs = await getRadioShackPairs();
