...
import { CETH, CHAINS, ChainId, Currency, Token } from '@radioshackswap/sdk2';
...
export function useCurrency(currencyId: string | undefined): Currency | null | undefined {
  const chainId = useChainId();
  const isAVAX = currencyId?.toUpperCase() === CETH[chainId].symbol?.toUpperCase();
  const useToken_ = useTokenHook[chainId];
  const token = useToken_(isAVAX ? undefined : currencyId);
  return isAVAX ? chainId && CETH[chainId] : token;
}
...
export function useCoinGeckoTokenPrice(coin: Token) {
  ...
  useEffect(() => {
    const getCoinPriceData = async () => {
      try {
        const chain = coin.chainId === ChainId.AVALANCHE ? CHAINS[ChainId.AVALANCHE] : CHAINS[coin.chainId];
